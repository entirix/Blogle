...
//OLD STUFF BELOW
	...
	function post_navigation() {
		echo '<nav class="pagination" role="pagination">';
		echo '	<span class="newer-posts">'.get_previous_posts_link('&laquo; Newer').'</span>';
		echo '	<span class="older-posts">'.get_next_posts_link('Older &raquo;').'</span>';
		echo '  <div class="page-number">'.paginate_links( $args ).'</div>';
		echo '</nav>';
	}

	// Posted On  | <span class="byline author vcard">%5$s</span>
	function posted_on() {
		printf( __( '', '' ),
	...
	// Navigation Page - update coming from twentythirteen
	function page_navigation() {
		echo '<nav class="pagination">';
		echo '<div class="article-nav">';
		echo '	<p class="next-article">'.next_post_link().'</p>';
		echo '	<p class="prev-article">'.previous_post_link().'</p>';
		echo '</div>';
		echo '</nav>';
	}
