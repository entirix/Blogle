...
<?php if ( have_comments() ) : ?>
	...
	<h2 id="comments" class="title"><?php comments_number(__('No Responses'), __('One Response'), __('% Responses') );?></h2>
 ...
 <?php else : // this is displayed if there are no comments so far ?>
	...
	<?php if ( comments_open() ) : ?>
	 ...
	 <?php else : // comments are closed ?>
		<p class="hidden"><?php _e('Comments are closed.','html5reset'); ?></p>
...
<div id="respond">
	<hr>
	<h2 class="comment-reply-link" align="center"><?php comment_form_title( 'Leave a Reply', __('Leave a Reply to %s') ); ?></h2>

	<div class="cancel-comment-reply">
		<h3 align="center"><?php cancel_comment_reply_link(); ?></h3>
	</div>
	...
	<form action="<?php echo get_option('siteurl'); ?>/wp-comments-post.php" method="post" id="commentform">
		...
		<?php else : ?>
			...
			<div class="clearfix">
				<p><label for="author"><?php _e('Name','html5reset'); ?> <?php if ($req) echo "(required)"; ?></label></p>
				<input type="text" name="author" id="author" value="<?php echo esc_attr($comment_author); ?>" size="22" tabindex="1" <?php if ($req) echo "aria-required='true'"; ?> />
			</div>

			<div class="clearfix">
				<p><label for="email"><?php _e('Mail (will not be published)','html5reset'); ?> <?php if ($req) echo "(required)"; ?></label></p>
				<input type="text" name="email" id="email" value="<?php echo esc_attr($comment_author_email); ?>" size="22" tabindex="2" <?php if ($req) echo "aria-required='true'"; ?> />
				
			</div>

			<div class="clearfix">
				<p><label for="url"><?php _e('Website','html5reset'); ?></label></p>
		...
		<div>
			<p><label for="url">Comment</label></p>
